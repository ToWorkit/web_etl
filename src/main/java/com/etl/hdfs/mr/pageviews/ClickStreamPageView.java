package com.etl.hdfs.mr.pageviews;

/**
 * 识别用户每次的会话(session)，以及每次session内访问了几步和每一步的停留时间[时间差](默认为60s)
 */

import com.etl.hdfs.mr.weblog.WebLogBean;
    ...
    static class ClickStreamReducer extends Reducer<Text, WebLogBean, NullWritable, Text> {
        ...
        protected void reduce(Text key, Iterable<WebLogBean> values, Context context) throws IOException, InterruptedException {
            ...
            try {
                ...
                // 将bean按时间先后顺序排序
                // new Comparator接口，对时间进行排序，需要判断前后两条数据的时间差
                ...
                for (int i = 0; i < beans.size(); i++) {
                    ...
                    if (1 == beans.size()) {
                        // 设置默认的停留时间60s
                        v.set(session + "\001" + key.toString() + "\001" + bean.getRemote_user() + "\001" + bean.getTime_local() + "\001" +
                                bean.getRequest() + "\001" + step + "\001" + (60) + "\001" + bean.getHttp_referer() + "\001" +
                    ...
                    // 如果不止1条数据，则将第一条跳过不输出(统计停留时间)，遍历第二条时再输出
                    if (i == 0) {
                        continue;
                    }
                    ...
                    // 如果本次-上次时间差<30分钟(同一个会话)，则输出前一次的页面访问信息
...
//		FileOutputFormat.setOutputPath(job, new Path(args[1]));
        ...
        FileInputFormat.setInputPaths(job, new Path("G:\\CODE\\data\\web_etl\\weblog\\output"));
        FileOutputFormat.setOutputPath(job, new Path("G:\\CODE\\data\\web_etl\\weblog\\output\\pageviews_01"));
